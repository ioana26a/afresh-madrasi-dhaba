...
export async function checkGameUi(context,output,url='http://127.0.0.1:5173/'){
 ...
 const gameControl=async name=>{const target=page.getByRole('button',{name,exact:true});await target.waitFor();const box=await target.boundingBox();await page.mouse.click(box.x+box.width/2,box.y+box.height/2);};
